...
export function CategoryModelComponent({
  ...
  return (
    <SelectableListComponent
      query={`SELECT c.ECInstanceId, COALESCE(c.UserLabel, c.CodeValue, 'Unnamed Category') as label FROM bis.SpatialCategory c WHERE c.ECInstanceId IN (SELECT DISTINCT Category.Id FROM bis.GeometricElement3d WHERE Category.Id IS NOT NULL) ORDER BY label`}
      labelKey="label"
      idKey="ECInstanceId"
